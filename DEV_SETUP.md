...
## 9. Component-2020 Integration Setup

Для настройки интеграции с Component-2020 (Access MDB):

1. Перейдите в **Настройки → Интеграции → Component-2020** (route: `/settings/integrations/component2020`).
2. Заполните:
   - путь к `.mdb` (поле «Путь к MDB»);
   - **login** (если требуется);
   - пароль (опционально).
     ВАЖНО: пароль **никогда не отображается обратно** в UI — показывается только факт наличия («Пароль задан»).
     Чтобы **не менять** пароль — оставьте поле пустым. Чтобы **очистить** сохранённый пароль — отметьте чекбокс «Очистить сохранённый пароль».
3. Нажмите «Проверить подключение».
4. Нажмите «Сохранить».
5. В блоке «Синхронизация» выберите Scope и режим:
   - `Dry-run` — без изменения данных MDM и без продвижения delta-курсорa;
   - `Commit` — изменение данных MDM и продвижение курсора при успехе.
6. Нажмите «Запустить». Историю запусков и ошибки смотрите в таблицах «Запуски» / «Ошибки синхронизации».

Синхронизация читает данные из Access MDB и обновляет MDM в PostgreSQL. Для тестов используйте Dry Run, чтобы не изменять данные.

Планировщик (Hosted Service) **не запускает синхронизацию сам по себе** — он начинает работать только при наличии активных расписаний, созданных через UI/API (scheduling).

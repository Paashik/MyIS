...
public sealed class CloseRequestHandler : RequestWorkflowActionHandlerBase
{
    ...
    public Task<RequestDto> Handle(CloseRequestCommand command, CancellationToken cancellationToken)
        => ExecuteAsync(command.RequestId, command.CurrentUserId, RequestActionCodes.Close, command.Comment, cancellationToken);
}

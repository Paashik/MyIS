...
public sealed class StartWorkOnRequestHandler : RequestWorkflowActionHandlerBase
{
    ...
    public Task<RequestDto> Handle(StartWorkOnRequestCommand command, CancellationToken cancellationToken)
        => ExecuteAsync(command.RequestId, command.CurrentUserId, RequestActionCodes.StartWork, command.Comment, cancellationToken);
}

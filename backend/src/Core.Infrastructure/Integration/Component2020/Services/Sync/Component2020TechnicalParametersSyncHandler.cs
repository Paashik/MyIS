...
public sealed class Component2020TechnicalParametersSyncHandler : IComponent2020SyncHandler
{
    ...
    private readonly ILogger<Component2020TechnicalParametersSyncHandler> _logger;

    public Component2020TechnicalParametersSyncHandler(
        ...
        ILogger<Component2020TechnicalParametersSyncHandler> logger)
    {
    ...
    public Component2020SyncScope Scope => Component2020SyncScope.TechnicalParameters;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var parameters = await _deltaReader.ReadTechnicalParametersDeltaAsync(connectionId, lastKey, cancellationToken);
        ...
        if (!dryRun)
        {
            var externalIds = parameters.Select(x => x.Id.ToString()).Distinct(StringComparer.Ordinal).ToList();
        ...
        foreach (var parameter in parameters)
        {
            try
            {
                var externalId = parameter.Id.ToString();
                ...
                if (existing == null)
                {
                    if (!dryRun)
                    {
                        ...
                        var created = new TechnicalParameter(parameter.Name, parameter.Name, parameter.Symbol, parameter.UnitId);
                ...
                else
                {
                    if (!dryRun)
                    {
                        existing.Update(parameter.Name, parameter.Symbol, parameter.UnitId, true);
                ...
                newLastKey = Math.Max(int.Parse(newLastKey ?? "0"), parameter.Id).ToString();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error syncing technical parameter {ParameterId}", parameter.Id);
                var error = new Component2020SyncError(runId, entityType, null, parameter.Id.ToString(), ex.Message, ex.StackTrace);
                errors.Add(error);
            }
        }

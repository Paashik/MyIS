...
3. Интеграции — только здесь (`IComponent2020Service`, импорт/экспорт, печатные формы).
   Детализированная структура доступов Компонент‑2020, которой должны следовать все реализации интеграций, приведена в [Component2020_Access_schema_mermaid.md](Component2020_Access_schema_mermaid.md).
...
Дополнение по суффиксам DTO:

- В **Application**: использовать суффиксы по назначению — `*Command`, `*Query`, `*ResponseDto`.
- В **WebApi** (контракты HTTP): использовать `*Request` / `*Response` (как модели запросов/ответов HTTP), не подменяя ими Application‑DTO.
...
Дополнение (устойчивость UI-автотестов):

- E2E/UI‑тесты не должны опираться на отображаемые тексты (тексты кнопок/ошибок/уведомлений).
- Предпочтение устойчивым селекторам: `data-testid`, `role`, `aria-*`, имена полей форм, URL/route.
- Если сравнение текста неизбежно — использовать централизованные i18n‑ключи/словарь RU как единый источник строк (см. `.kilocode/rules/memory-bank/guidelines.md`).
...
# 9. Управляемые справочники и параметры (модуль Settings)

Этот раздел фиксирует правила для **настроечных данных** (справочники, статусы, типы, матрицы переходов workflow, перечни причин/категорий/кодовых значений), которые должны управляться без правок кода.

### Обязательно

1. Любая настроечная информация, которая:
   - влияет на бизнес‑поведение модуля, **или**
   - должна изменяться без изменения кода, **или**
   - используется более чем в одном модуле,
   должна оформляться как **управляемая настройка/справочник** и обслуживаться через **единый UI‑модуль `Settings`**.
2. Справочники и настройки **живут в домене‑владельце** (своя схема/таблицы домена; пример: `requests.request_types`, `requests.request_statuses`).
3. Для справочников обязателен **стабильный код** (`Code` / `code`) как ключ интеграции и UI‑логики: нельзя завязываться на GUID как на бизнес‑ключ.
4. Физическое удаление справочных записей **запрещено**, если на них есть ссылки: используется `IsActive`/архивация.

### Желательно

- Типо‑/статусо‑зависимая логика (включая UI‑условия отображения) должна опираться на `Code`, а не на «магические» строки/числа и не на `Id`.
- Для workflow‑переходов хранить `ActionCode` и (по возможности) `RequiredPermission`, чтобы UI мог получать допустимые действия от backend и не повторять правила переходов.

### Запрещено

- Хардкодить справочники (типы/статусы/причины/категории) во Frontend в виде констант, если они предполагаются изменяемыми.
- Разносить административные экраны по доменам так, что пользователь вынужден искать настройки в разных местах: все настройки должны быть собраны в `Settings`.
